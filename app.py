...
code_history = db["code_history"]   # ← new collection
...
def save_history():
    ...
    # Keep only latest 30 entries per user to avoid unbounded growth
    all_ids = [
        doc["_id"] for doc in code_history.find(
            {"username": username}, {"_id": 1}
        ).sort("timestamp", -1)
    ]
    if len(all_ids) > 30:
        code_history.delete_many({"_id": {"$in": all_ids[30:]}})
    return jsonify({"status": "success"})
...
def get_history():
    ...
    docs = list(
        code_history.find(
            {"username": username},
            {"_id": 1, "language": 1, "filename": 1, "code": 1, "timestamp": 1}
...
@app.route("/delete-history/<entry_id>", methods=["DELETE"])
def delete_history(entry_id):
    username = session.get("user")
    if not username:
        return jsonify({"status": "error"}), 401
    from bson import ObjectId
    code_history.delete_one({"_id": ObjectId(entry_id), "username": username})
    return jsonify({"status": "success"})
